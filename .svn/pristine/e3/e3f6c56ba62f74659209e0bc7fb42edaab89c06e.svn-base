...
namespace Test
{
    ...
    //public const string FILE_INI = Form1.PATH + "TestBoard.ini"; //Tiene la dependencia de public string PATH en From1
        ...
        private const int REINTENTOS = 10;
        //Variables
        ...
        //Objetos
        SerialPort serialPort1 = new SerialPort("COM3");    //Puerto serie
        IniManager m_inimanager = new IniManager(FILE_INI); //Archivo de configuracion .ini     
        #endregion

        
        #region Constructores
        public TestBoard(string COM)
        {
            serialPort1.PortName = m_inimanager.ReadValue("GEN", "port", COM);
        ...
        public bool leerByte(byte id, byte variable_L, out byte valor)
        {
            ...
            trama[6] = LRC(trama);
            envioTrama(trama);
            recibido = (waitResponse());    //Espera a que responda y almacena el resultado en la variable
            if(recibido)
        ...
        public bool escribirByte(byte id, byte variable_L, byte valor)
        {
            ...
            while (!recibido && reintentos < REINTENTOS)
            {
                ...
                ClearBytes();
                envioTrama(trama);
                recibido = (waitResponse());    //Espera a que responda
                reintentos++;
            }
        ...
        public bool leerByteBucle(byte id, byte idOperador, byte variable_H ,byte variable_L, out byte valor)
        {
            ...
            while (!recibido && reintentos < REINTENTOS)
            {
                ...
                ClearBytes();   //Limpia el contenido del mensaje anterior
                envioTrama(trama);
                recibido = (waitResponse());    //Espera a que responda y almacena el resultado en la variable
        ...
        public bool leerByteADIIBucle(byte id, byte idOperador, EnumTB.ParametroADII param ,out byte valor)
        {
            ...
            while (!recibido && reintentos < REINTENTOS)
            {
                ...
                envioTrama(trama);
                recibido = (waitResponse());    //Espera a que responda y almacena el resultado en la variable
                if (!extraerByteADIISinId(out temp)) //Extrae el valor recibido en la trama
                    recibido = false;
                reintentos++;
            }
            valor = temp;
        ...
        public bool leerWordADIIBucle(byte id, byte idOperador, EnumTB.ParametroADII param, out UInt16 valor)
        {
            ...
            while (!recibido && reintentos < REINTENTOS)
            {
                ...
                envioTrama(trama);
                recibido = (waitResponse());    //Espera a que responda y almacena el resultado en la variable
                if (!extraerWordADIISinId(out temp)) //Extrae el valor recibido en la trama
                    recibido = false;
                reintentos++;
            }
            valor = temp;           
        ...
        private void serialPort1_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
        {
            ...
            while (StopReceive); //Se detiene aqui hasta que se abra el semaforo
            Thread.Sleep(40); //Espera para recibir toda la trama antes de configurar buffer era 22
            try
            {
        ...
        /// <summary>
        /// Comprueba si ha llegado el mensaje de respuesta del circuito TestBoard
        /// </summary>
        /// <returns></returns>
        private bool waitResponse()
        {
            EndReceive = true;
            int i = 0;
            while (EndReceive & i < 150) //Espera a que se apague el semaforo o a el timeout 150 antes 100
            {
                Thread.Sleep(1);
                i++;
            }
        ...
        private bool extraerByte(byte id, out byte resultado)
        {
            ...
            if (TBReceive[0] == STX && TBReceive[2] == 5 && tramaByteOk(id)) //Comprueba que la trama sea correcta
            {                
                resultado = TBReceive[6]; //Devuelve el resultado y 
        ...
        /// <param name="resultado">Devuelve el resultado como argumento de salida</param>
        /// <returns>Devuelve true si la trama ha sido correcta</returns>
        private bool extraerByteADIISinId(out byte resultado)
        {
            StopReceive = true; //Detiene la recepcion para que no se modifique la variable
            //if (TBReceive[0] == STX & TBReceive[1] == id & TBReceive[2] == 5 & tramaByteOk(id)) //Comprueba que la trama sea correcta
            if (TBReceive[0] == STX && TBReceive[2] == 4 ) //Comprueba que la trama sea correcta
            {
                resultado = TBReceive[5]; //Devuelve el resultado y 
                StopReceive = false; //Vuelve a dejar libre la comunicación
                return true; //Devuelve true si la trama es correcta
            }
            else
        ...
        {
            StopReceive = true; //Detiene la recepcion para que no se modifique la variable
            if (TBReceive[0] == STX && TBReceive[1] == id && TBReceive[2] == 6 && tramaWordOk(id)) //Comprueba que la trama sea correcta
            {
                byte[] datoCovertir = new byte[2];//Crea una matriz de 2 bytes y les invierte el orden para sacar el valor entero                
                datoCovertir[0] = TBReceive[7];
                datoCovertir[1] = TBReceive[6];
        ...
        private bool extraerBuffer(byte id, byte direccion,out byte[] resultado)
        {
            byte[] bufferRecibido = new byte[16];
            StopReceive = true; //Detiene la recepcion para que no se modifique la variable
            if (TBReceive[0] == STX && TBReceive[1] == id && TBReceive[2] == 21 && TBReceive[5] == direccion && tramaBufferOk(id)) //Comprueba que la trama sea correcta
            {
                for (int i = 0; i < 16; i++)
                {
                    bufferRecibido[i] = TBReceive[i + 7];
        ...
        /// <param name="resultado">Devuelve el resultado como argumento de salida</param>
        /// <returns>Devuelve true si la trama ha sido correcta</returns>
        private bool extraerWordADIISinId(out UInt16 resultado)
        {
            StopReceive = true; //Detiene la recepcion para que no se modifique la variable
            if (TBReceive[0] == STX & TBReceive[2] == 5 ) //Comprueba que la trama sea correcta
            {
                byte[] datoCovertir = new byte[2];//Crea una matriz de 2 bytes y les invierte el orden para sacar el valor entero                
